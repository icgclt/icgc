...
export default function MemberPortal() {
  ...
  const myRegistrations = useMemo(() => member ? (data.event_registrations||[]).filter(r=>r.member_id===member.id) : [], [data.event_registrations,member]);
  ...
  async function register(event){
    if(!member) return;
    if(myRegistrations.some(r=>r.event_id===event.id && r.status!=='Cancelled')) return;
    const {error}=await supabase.from('event_registrations').insert({id:crypto.randomUUID(),event_id:event.id,member_id:member.id,member_name:member.name,registered_at:new Date().toISOString(),status:'Registered'});
    if(error){alert(error.message);return;} await syncNow();
  }
  async function cancelRegistration(r){
    const {error}=await supabase.from('event_registrations').update({status:'Cancelled'}).eq('id',r.id); if(error){alert(error.message);return;} await syncNow();
  }
  ...
  return <div>
    ...
    {tab==='home' && <>
      ...
      <div className="panel"><h2>Quick actions</h2><div className="toolbar"><button className="primary" onClick={()=>setTab('profile')}>Update my profile</button><button className="secondary" onClick={()=>setTab('pay')}>Give by Mobile Money</button><button className="secondary" onClick={()=>setTab('events')}>Register for an event</button><button className="secondary" onClick={()=>setTab('prayer')}>Send prayer request</button></div></div>
    ...
    {tab==='events' && <div className="panel"><h2>Upcoming events</h2>{events.map(e=>{const reg=myRegistrations.find(r=>r.event_id===e.id && r.status!=='Cancelled');return <div className="listrow" key={e.id}><b>{e.title}</b><div>{e.date} {e.event_time||''} {e.location?`• ${e.location}`:''}</div><div className="muted">{e.description||''}</div>{reg?<button className="secondary" onClick={()=>cancelRegistration(reg)} style={{marginTop:8}}>Cancel registration</button>:<button className="primary" onClick={()=>register(e)} style={{marginTop:8}}>Register</button>}</div>})}{!events.length&&<p className="muted">No upcoming events.</p>}<h2 style={{marginTop:24}}>My registrations</h2>{myRegistrations.length?<div className="tablewrap"><table><thead><tr><th>Event</th><th>Status</th><th>Registered</th></tr></thead><tbody>{myRegistrations.map(r=><tr key={r.id}><td>{data.events.find(e=>e.id===r.event_id)?.title||r.member_name||'Event'}</td><td>{r.status}</td><td>{dateText(r.registered_at||r.created_at)}</td></tr>)}</tbody></table></div>:<p className="muted">No event registrations yet.</p>}</div>}
